...
5.24 目前bug：1.图片有时加载不出来  2.展示相册第二次点击就开始异常动作。
...
puzzleGame.prototype = {
	...
	imgSplit:function(){
		...
		//例如第一次拆分3x3,插入了9个div，但没有清空，第二次拆分4x4，此时是在前9个div之后再插入16个div，共9+16个div
		this.imgArea.html("");
		...
		this.imgCells = $('#wrap #right #imgArea div.imgCell');//碎片节点（这里可以把“div”去掉，一样）
	},
	...
	levelSelect:function(){
		...
		this.btnLevel.bind('mousedown',function(){
			...
			// 	self.levelNow = 0;
			// }
			self.levelNow = (self.levelNow + 1) % len;
			//显示的难度改变
		...
		this.btnStart.bind('mousedown',function(){
			...
			if(self.hasStart == 0){//不在游戏中
				...
				self.imgCells.css({
					...
					var cell_mouse_x = e.pageX - self.imgCells.eq(cellIndex_1).offset().left;//获取当前鼠标相对选中碎片的X坐标  
					var cell_mouse_y = e.pageY - self.imgCells.eq(cellIndex_1).offset().top;//获取当前鼠标相对选中碎片的y坐标  
                      
					$(document).bind('mousemove',function(e2){
						self.imgCells.eq(cellIndex_1).css({
							'z-index':'40', //“源代码值为40”z-index 属性设置元素的堆叠顺序。拥有更高堆叠顺序的元素总是会处于堆叠顺序较低的元素的前面。
							'left':(e2.pageX - cell_mouse_x - self.imgArea.offset().left) + 'px',//鼠标在x坐标上的偏移距离
							'top':(e2.pageY - cell_mouse_y - self.imgArea.offset().top) + 'px',//鼠标在y坐标上的偏移距离
						});
	...
	cellChangeIndex:function(x,y,orig){
		...
		//鼠标拖动碎片在大图范围内移动 ?:1.为何不直接返回location 2.为何不返回imgRandArr[location]?
		/*
		   需要明确的是，数组中每个元素存的是位置（从0号碎片），数组下标代表了每个碎片。
		   ！！！！而不能理解为【数组中的元素存着碎片原始编号，位置为下标】！！！
		*/
	...
	*/
	nextPhoto:function(){
		...
		var self = this;
        this.btnPhoto.bind('mousedown',function(){
			$(this).addClass('mouseOn');
		}).bind('mouseup',function(){
			$(this).removeClass('mouseOn');
		}).bind('click',function(){
			...
			var len = self.photoArr.length;
            self.photoIndex = (self.photoIndex + 1) % len;
			self.img  = self.photoArr[self.photoIndex] 
			self.imgSplit();
	...
	showPhoto:function(){
		...
		var self = this;
		this.btnAlbum.bind('mousedown',function(){
			$(this).addClass('mouseOn');
		}).bind('mouseup',function(){
			$(this).removeClass('mouseOn');
		}).bind('click',function(){
